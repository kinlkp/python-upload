...
and check the NetBackup status of the hosts
"""
...
class NBUClient:
    ...
    NBU_SERVERs = {
        "p1": "p1psmwindc003.prod.abc.local",
        "p2": "p2psmwindc003.prod.abc.local",
        "n1": "n1psmwindc003.nonprod.abc.local",
        "n2": "n2psmwindc003.nonprod.abc.local",
    }
...
class SiteScopeClient():
    ...
    def __init__(self, row):
        self.row = row
        try:
            self.time_delta = int(sys.argv[1])
        except:
            self.time_delta = 1         # means delta is 1 day
    ...
    def blackout(self):
        ...
        if this_host[0] == 'n' and host[0] == 'n':
            web_URL = 'https://n1vsmobmaw0010.nonprod.abc.local/SiteScope'
        elif this_host[0] == 'p' and host[0] == 'p':
            web_URL = 'https://p1vsmobmaw0010.prod.abc.local/SiteScope'
        elif this_host[0] == 'p' and host[0] == 'o':
            web_URL = 'https://p1vsmobmaw0010.prod.abc.local/SiteScope'
        else:
            return 'Cannot find sitescope'
        ...
        today = datetime.now()
        tmr = today + timedelta(self.time_delta)
        ...
        if not bool(groups_dict):
            return f"{host} is NOT in sitescope."
        ...
        for k, group_name in enumerate(groups_dict):
            if groups_dict[group_name] == 'Group' and host in group_name:
                ...
                if response.status_code == 204:
                    ...
                    return f"{start_suppress} for {blackout_duration/1000/60} mins"
                else:
...
def main():
    ...
    print("{:<30s} {:<50s} {:<30s}".format("Hostname", "Sitescope starts blackout", "Last Netbackup"))
    import csv
    with open('/tmp/servers.csv', "r") as csv_f:
        for row in csv_f:
            ...
            print("{:<30s} {:<50s} {:<30s}".format(hostname, sitescope_output[-22:], nbu_output[-10:]))
